        ...
        if (strlen($_POST["freeword"]))
        {
            ...
            foreach ($freeword as $val)
            {
                ...
                else
                {
                    ...
                    $sql .= " OR replace(replace(replace(pt.subject, '-', ''), '/', ''), '.', '') LIKE '%{$val}%'";
                }

                //$sql .= " OR pt.location LIKE '%{$val}%'
                //      OR pt.content LIKE '%{$val}%'";
                $sql .= " OR pt.location LIKE '%{$val}%'
                      OR replace(replace(replace(pt.content, '-', ''), '/', ''), '.', '') LIKE '%{$val}%'";
...
function listStr($project_data)
{
    ...
    foreach ($project_data as $val)
    {
            ...
            $msg .="
                    <div class='project_listbox bg-gray ".newStamp($val["post_date"])."'>
	                    <div>
	                        ...
	                        <div class='project_member'><span class='color-member'>【会員様限定！】{$val["subject"]}</span></div>
	                        <div class='clear'></div>
	                    </div>
                        ...
                        <div class='project_content color-member bg-white'>こちらは会員様限定案件です。
                            <br><a href='".Fj_LogIn."'>→ログインはこちら</a>
                            <br><a href='".Fj_NewMember."'>→無料新規会員登録はこちら</a>
                        </div>
                    </div>";
        }
        else
        {
            ...
            {
                $mark = "【会員様限定！】";
                $memberCss = "project_listbtn_member";
            ...
            $msg .= "
                    <div class='project_listbox bg-gray ".newStamp($val["post_date"])."'>
                        <div>
                        	<div class='listbox_post_date'>掲載日：{$val["post_date"]}</div>
                            <div class='project_listbtn {$memberCss}'>
                            <a href='".Fj_Project."?page=".($val["project_id"])."'>
                                {$mark}{$val["subject"]}
                            </a></div>
                            <div class='clear'></div>
                        </div>
                        ...
                        <div class='project_content bg-white'>
                            <div class='project_text_continue'>".nl2br($val["content"])."</div>
                            <a href='".Fj_Project."?page=".($val["project_id"])."'>→続きを読む</a>
                        </div>
                    </div>
                    ";
        }
...
<head>
    ...
    <script>
        jQuery(function($) {
  $('.project_text_continue').each(function() {
    var $target = $(this);
